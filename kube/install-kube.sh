...
sudo -E apt update
sudo -E install -y docker.io kubelet="${KUBE_VERSION}" kubeadm="${KUBE_VERSION}" \
...
curl -sSL https://storage.googleapis.com/kubernetes-helm/helm-${HELM_VERSION}-linux-amd64.tar.gz | tar -zxv --strip-components=1 -C ${TMP_DIR}sudo mv ${TMP_DIR}/helm /usr/local/bin/helm
rm -rf ${TMP_DIR}
